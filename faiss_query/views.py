    ...
    if request.method == "POST":
        ...
        if olay:
            ...
            if cached_aciklama:
                ...
                try:
                    # Mevzuat entegrasyonu - kanun numaralarını tespit et
                    from .mevzuat_integration import extract_law_numbers, get_updated_law_info, create_enhanced_prompt
                    
                    law_numbers = extract_law_numbers(olay)
                    law_info = {}
                    
                    # Eğer kanun numarası varsa güncel bilgiyi al
                    if law_numbers:
                        try:
                            law_info = get_updated_law_info(law_numbers)
                        except Exception as e:
                            print(f"Mevzuat bilgisi alınamadı: {e}")
                    
                    # Zenginleştirilmiş prompt oluştur
                    prompt = create_enhanced_prompt(olay, law_info)
