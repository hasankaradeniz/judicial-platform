# faiss_query/views.py - FINAL REAL VERSION
...
def extract_legal_keywords(text):
    ...
    keywords = []
    miras_keywords = ['miras', 'vasiyet', 'tereke', 'mirasçı', 'veraset', 'gayrimenkul', 'devir', 'iptal']
    for keyword in miras_keywords:
        if keyword.lower() in text.lower():
            keywords.append(keyword)
    return keywords if keywords else ['hukuk']

def olay_gir(request):
    """Input form for legal case"""
    if request.method == 'POST':
        olay = request.POST.get('olay', '').strip()
        if olay:
            request.session['olay'] = olay
            request.session['hukuki_aciklama'] = 'Hierarchical FAISS ile arama yapıldı'
...
def karar_detay(request, alan, index):
    """Real case detail view"""
    olay = request.session.get('olay')
    if not olay:
        return redirect('faiss_query:olay_gir')

    # Simple detail view for real cases
    return render(request, 'faiss_query/karar_detay.html', {
        'karar': {
            'text': 'Gerçek karar detayı yükleniyor...', 
            'mahkeme': 'Gerçek Mahkeme',
            'tarih': '2024',
            'sayi': index
        },
        'alan': alan,
