# param_soap_payment.py - Param Desteğinin Belirttiği SOAP Endpoint ile Ödeme
...
class ParamSoapPayment:
    def __init__(self):
        ...
        # Param desteğinin belirttiği endpoint
        self.soap_url = "https://posws.param.com.tr/turkpos.ws/service_turkpos_prod.asmx"
        
        logger.info(f"ParamSoapPayment initialized - Test Mode: {self.test_mode}")
        logger.info(f"Using SOAP URL: {self.soap_url}")
    ...
    def process_payment(self, request, card_data, payment_data):
        ...
        try:
            ...
            logger.info(f"SOAP Payment Debug:")
    ...
    <Pos_Odeme xmlns="https://turkpos.com.tr/">
      ...
      <Data5></Data5>
            ...
            if response.status_code == 200:
                logger.info(f"SOAP Response Body: {response.text[:1000]}...")
                ...
                try:
                    ...
                    if result is not None:
                        ...
                        logger.error(f"SOAP Response parsing failed. Full response: {response.text}")
                        return {
                            'success': False,
                    ...
                    return {
                        'success': False,
                        'error': f'XML ayrıştırma hatası: {str(parse_error)}'
                    }
            else:
                logger.error(f"SOAP HTTP Error - Status: {response.status_code}")
                logger.error(f"SOAP Error Response: {response.text}")
                return {
                    'success': False,
                    'error': f'SOAP HTTP hatası: {response.status_code} - {response.text[:200]}'
                }
            ...
            return {
                'success': False,
                'error': f'Bağlantı hatası: {str(req_error)}'
            }
