...
def search_results_view(request):
    ...
    if query:
        ...
        # Veritabanı sorgusu
        if q_objects:
            results = JudicialDecision.objects.filter(q_objects).select_related()
            
            # Sıralama
            if sort_order == 'date':
                results = results.order_by('-karar_tarihi')
            else:
            ...
            # Sayfalama
            paginator = Paginator(results, 20)  # Sayfa başına 20 sonuç
            page_obj = paginator.get_page(page_number)
