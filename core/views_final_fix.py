def judicial_decisions(request):
    ...
    if has_any_filter:
        ...
        # Build WHERE conditions and parameters carefully
        where_conditions = []
        search_params = []
        count_params = []
        ...
        if court_type:
            if court_type.lower() == 'yargitay':
                ...
                search_params.append('%YARGITAY%')
                count_params.append('%YARGITAY%')
            elif court_type.lower() == 'danistay':
                where_conditions.append("UPPER(karar_turu) LIKE %s") 
                search_params.append('%DANİŞTAY%')
                count_params.append('%DANİŞTAY%')
            elif court_type.lower() == 'aym':
                where_conditions.append("UPPER(karar_turu) LIKE %s")
                search_params.append('%ANAYASA%')
                count_params.append('%ANAYASA%')
            elif court_type.lower() == 'bolge':
                where_conditions.append("UPPER(karar_turu) LIKE %s")
                search_params.append('%BÖLGE%')
                count_params.append('%BÖLGE%')
            else:
                where_conditions.append("UPPER(karar_turu) LIKE %s")
                search_params.append(f'%{court_type.upper()}%')
                count_params.append(f'%{court_type.upper()}%')
        
        # Chamber filter
        if chamber:
            where_conditions.append("UPPER(karar_veren_mahkeme) LIKE %s")
            search_params.append(f'%{chamber.upper()}%')
            count_params.append(f'%{chamber.upper()}%')
        
        # Decision number filter
        if decision_number:
            where_conditions.append("karar_numarasi ILIKE %s")
            search_params.append(f'%{decision_number}%')
            count_params.append(f'%{decision_number}%')
        
        # Case number filter
        if case_number:
            where_conditions.append("esas_numarasi ILIKE %s")
            search_params.append(f'%{case_number}%')
            count_params.append(f'%{case_number}%')
        ...
        if date_range and not date_from and not date_to:
            ...
            if date_range == 'last_6_months':
                ...
                where_conditions.append("karar_tarihi >= %s")
                search_params.append('2020-01-01')
                count_params.append('2020-01-01')
            elif date_range == '2015_onwards':
                where_conditions.append("karar_tarihi >= %s")
                search_params.append('2015-01-01')
                count_params.append('2015-01-01')
        ...
        with connection.cursor() as cursor:
            ...
            if query:
                ...
                search_sql = f"""
                    ...
                    LIMIT %s OFFSET %s
                """
                # Add query param again for ranking function
                final_search_params = [query] + search_params
            else:
            ...
            # Execute search
            final_search_params.extend([per_page, offset])
            
            try:
                cursor.execute(search_sql, final_search_params)
                results = cursor.fetchall()
                
                # Execute count
                cursor.execute(count_sql, count_params)
                count_result = cursor.fetchone()
                total_results = count_result[0] if count_result else 0
                
            except Exception as e:
                print(f"Database error: {e}")
                print(f"Search SQL: {search_sql}")
                print(f"Search params: {final_search_params}")
        ...
        cache_key_search_stats = f'search_stats_{hash(str(query + str(court_type) + str(chamber)))}'
            ...
            try:
                with connection.cursor() as cursor:
                    stats_sql = f"""
                        ...
                        GROUP BY karar_turu
                    """
                    cursor.execute(stats_sql, count_params)
