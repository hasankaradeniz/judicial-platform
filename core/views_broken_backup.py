...
def judicial_decisions(request):
    ...
    if query or court_type or chamber or date_range or decision_number or case_number:
        ...
        with connection.cursor() as cursor:
            ...
            where_clauses = []
            where_params = []  # WHERE koşulları için ayrı parametre listesi
            params = []
            ...
            if query:
                ...
                where_params.append(query)  # sadece where için
            ...
            if date_range:
                ...
                if date_range == 'today':
                    ...
                    params.append(today)
                    where_params.append(today)
                elif date_range == 'week':
                    where_clauses.append("karar_tarihi >= %s")
                    params.append(today - timedelta(days=7))
                    where_params.append(params[-1])
            ...
            count_params = []  # Initialize count_params
        ...
        if court_type_counts is None:
            with connection.cursor() as cursor:
                ...
                if where_sql:
                    cursor.execute(stats_sql, where_params)
                else:
...
def subscription_payment(request, package):
    ...
    if request.method == "POST":
        # Kart bilgisi gönderiliyorsa, diğer bilgileri session'dan al
        if request.POST.get('card_number'):
            ...
            accepted_sale = user_data.get('accepted_sale', False)
            accepted_delivery = user_data.get('accepted_delivery', False)
            tc_or_vergi_no = user_data.get('tc_or_vergi_no', '')
        ...
        # Sözleşmelerin hepsi kabul edilmediyse hata göster (sadece kart bilgisi yoksa)
        if not request.POST.get('card_number') and not (accepted_terms and accepted_sale and accepted_delivery):
            error = "Lütfen tüm sözleşmeleri kabul edin."
            return render(request, 'core/subscription_payment.html', {'error': error, 'package': package})

        # Gerekli alanlar doldurulmuş mu kontrol et (kart bilgisi yoksa)
        if not request.POST.get('card_number') and not all([tc_or_vergi_no, address, customer_name, customer_phone]):
        ...
        # İlk aşama - kart formu göster veya işle
        if request.POST.get('card_number'):
            # Kart bilgileri gönderilmiş, ödemeyi işle
            card_data = {
            ...
            payment_data = request.session.get('pending_payment', {})
            payment_data['package_type'] = package
            payment_data['amount'] = param_service.get_package_amount(package)
            
            payment_result = param_service.start_payment(request, package, card_data)
        else:
            # İlk giriş - kart formu gösterilecek
            payment_result = param_service.start_payment(request, package, {})
        ...
        if payment_result['success']:
            if payment_result.get('is_form'):
                # Form-based payment redirect
                return render(request, 'core/payment_redirect_new.html', {
                    'payment_url': payment_result['payment_url'],
                    'form_params': payment_result['form_params']
                })
