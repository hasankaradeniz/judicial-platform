...
print("Model yüklendi, veritabanına bağlanılıyor...")
# Alan istatistiklerini al - \!= yerine <> kullan
query_stats = '''
    ...
    ORDER BY count DESC
'''
cursor.execute(query_stats, (MIN_TEXT_LENGTH,))
area_stats = cursor.fetchall()
print(f"Toplam {len(area_stats)} alan bulundu, detaylar: {area_stats[:3]}...")
...
for area_name, area_count in area_stats:
    ...
    query_decisions = '''
        ...
        ORDER BY id
    '''
    cursor.execute(query_decisions, (area_name, MIN_TEXT_LENGTH))
