...
batch_size = 500  # Daha küçük batch size
...
for offset in range(0, total, batch_size):
    ...
    # Her batch sonunda ilerleme göster
    batch_time = time.time() - batch_start
    total_time = time.time() - start_time
    avg_speed = processed / total_time
    eta = (total - processed) / avg_speed if avg_speed > 0 else 0
    
    print(f'Batch {offset//batch_size + 1}: {processed}/{total} ({(processed/total)*100:.1f}%) - Hız: {avg_speed:.0f} karar/sn - ETA: {int(eta//60)}dk {int(eta%60)}sn')
    
    if processed % 5000 == 0:
        top_areas = dict(sorted(area_counts.items(), key=lambda x: x[1], reverse=True)[:3])
        print(f'  📈 En çok tespit edilen alanlar: {top_areas}')

print(f'\n🎉 İşlem tamamlandı\!')
print(f'⏰ Bitiş: {datetime.now()}')
print(f'⏱️  Toplam süre: {int((time.time()-start_time)//60)} dakika')
print(f'\n📊 Final alan dağılımı:')
for area, count in sorted(area_counts.items(), key=lambda x: x[1], reverse=True):
    print(f'  - {area}: {count} karar ({(count/total)*100:.1f}%)')
